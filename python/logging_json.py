...
コンテナイメージデプロイのため Layer が使えず、ランタイムのログ形式設定にも
依存しないよう、フォーマッタを自前で持つ。

ルートロガーに設定することで、botocore / urllib3 / kubernetes など
ライブラリのログも同じ JSON 形式で出る。CloudWatch Logs Insights で
JSON フィールドを条件にクエリしたときに、ライブラリのログ行だけ
パースできないという事態を避けるため。
"""
