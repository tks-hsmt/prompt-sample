...
- [テンプレートと設定値の責務分離](#テンプレートと設定値の責務分離)
- [環境別 values ファイルの分離](#環境別-values-ファイルの分離)
- [合理的なデフォルト値の提供](#合理的なデフォルト値の提供)
- [変数の命名規則（camelCase）](#変数の命名規則camelcase)
- [フラット構造 vs ネスト構造](#フラット構造-vs-ネスト構造)
- [型の明示（文字列のクォート）](#型の明示文字列のクォート)
- [--set フレンドリーな設計](#--set-フレンドリーな設計)
- [values.yaml のコメント規則](#valuesyaml-のコメント規則)
- [values.yaml のベーステンプレート](#valuesyaml-のベーステンプレート)
- [values.schema.json によるバリデーション](#valuesschemajon-によるバリデーション)
