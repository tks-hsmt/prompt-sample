...
以下がユーザーの依頼から読み取れない場合、作業に着手せず確認する。回答の中で新たな不明点が出てきたら追加で確認してよい。
...
|---|---|
| `chart-structure.md` | チャート名は命名規則に準拠しているか。version は SemVer 2 か。ディレクトリ名とチャート名は一致しているか。**スキャフォールド差分チェック**: `helm create` が生成する標準ファイル構造と比較し、不足しているファイルがないか |
