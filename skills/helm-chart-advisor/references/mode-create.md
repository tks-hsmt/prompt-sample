...
作業を始める前に、以下の情報がユーザーの依頼から読み取れるか確認する。不足している項目があれば、作業に着手せずまとめてヒアリングする。すでに明示されている項目は聞き直さない。回答の中で新たな不明点が出てきたら追加で確認してよい。ヒアリング後、作業に着手する前に `confirmation-rules.md` のルールに従って確認をユーザーに提示し、OK を得てから進める。
...
- **リソース制限**: CPU/メモリの requests/limits（デフォルト: requests CPU 100m / メモリ 128Mi、limits CPU 500m / メモリ 256Mi）
...
4. **スキャフォールド生成**: `helm create <chart-name>` を実行し、標準ファイル構造を生成する（標準構造の詳細: `chart-structure.md`）
5. **生成ファイルの調整**: 手順4で生成されたファイルを、プランの要件に合わせて以下のとおり調整する:
   - `Chart.yaml` — SemVer 2 バージョニング（詳細: `chart-structure.md`）
   - `values.yaml` — 合理的なデフォルト値、camelCase 命名、パラメータ名で始まるコメント（詳細: `values.md`）
   - `values.schema.json` — 新規追加。入力バリデーション（詳細: `values.md`）
