...
values のキーは、対応する Kubernetes API の階層に合わせて命名・構造化する。共通的に利用されるものに限らず、特定ワークロードでのみ使われるキーも含めて、**名前と構造は以下の通りに固定する**。新しいチャートを起こす場合も別名(`additionalEnv`, `commonLabels`, `imageRepository` 等)に変更してはならない。
