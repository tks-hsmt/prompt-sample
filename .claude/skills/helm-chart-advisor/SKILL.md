...
対象チャートの全ファイルを読み込み、`references/` 配下の**すべてのリファレンスファイル**を読み込んだうえでレビューを行う。判定は対象チャートの現在の内容とリファレンスのルールのみに基づいて行う。`results/` 配下の過去のレビュー結果や計画ファイルは読み込まない。過去の判定に影響されず、毎回独立した評価を行うことが正確なレビューには不可欠である。
...
以下は作成モードで **必ず読み込む**。チャートの種別によらず、すべての作成作業で必要になる。

| ファイル | 内容 |
|---|---|
| `chart-files.md` | ディレクトリ構成とファイル配置ルール |
| `values.md` | キー命名、構造、標準キー一覧、コメントルール |
| `templates.md` | テンプレート構造、ヘルパー関数、フォーマット |
| `labels.md` | 必須ラベル、セレクタ、アノテーション |
| `pods-basic.md` | イメージ指定方式、pullPolicy、セレクタ |
| `pods-security.md` | securityContext、capabilities、ホスト名前空間 |
| `pods-reliability.md` | resources、probes、topologySpreadConstraints |
| `values-environment.md` | 環境別ファイルの構成と差分ルール |
| `documentation.md` | helm-docs、NOTES.txt のルール |

以下はヒアリングの結果、該当する場合に読み込む。

| 条件 | ファイル | 内容 |
|---|---|---|
| 外部リソースへのアクセスが必要 | `rbac-basic.md` | ServiceAccount、Role/RoleBinding 構造 |
| RBAC 権限の設計が必要 | `rbac-least-privilege.md` | 最小権限の原則、verb/resource の選定 |
