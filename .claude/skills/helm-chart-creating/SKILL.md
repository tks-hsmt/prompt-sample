...
2. **ワークロード種別をユーザーにヒアリング** (未明示なら必ず質問、推論禁止):
   ```
   ワークロード種別を教えてください:
...
3. **対象 k8s クラスタのバージョンをユーザーにヒアリング** (例: "EKS 1.30")

4. **ワークロード種別、k8sクラスタのバージョン、リクエスト内容からアプリ特性を抽出**: アプリ種類、入出力、状態保持、HA 要件、特殊配置要件 等

5. **ワークロード種別に沿った`workload-basics/<workload>.md` を読み込む**: ワークロード特有の能動的提案を実行
